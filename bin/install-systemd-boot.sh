...
arch-chroot /mnt /bin/bash -c "bootctl install --esp-path=$ESP"
...
cat "./customs/systemd-boot-uki-kernel.conf" | sed "s|ESP|$ESP|g" | sed "s|linux|$kernel_to_install|g" | tee "/mnt/etc/mkinitcpio.d/""$kernel_to_install"".preset.pacsave"
